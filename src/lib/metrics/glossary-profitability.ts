...
export const profitabilityMetrics = {
  ...
  // ─────────────────────────────────────────────────────────────
  // v2 WS3: 박리다매 엔진의 실체 — 단위공헌이익
  // ─────────────────────────────────────────────────────────────
  unit_contribution_margin: {
    id: "unit_contribution_margin",
    name: "단위 공헌이익",
    category: "profitability",
    unit: "currency",
    formula:
      "단위 공헌이익 = 판매단가 − 단위 변동비\n" +
      "(변동비 = 원재료 + 노무 + 외주 + 기타 변동 원가; 고정비는 제외)\n" +
      "추가 판매 1개당 실제로 늘어나는 이익을 의미합니다.",
    beginner:
      "🍜 한 그릇 더 팔 때 진짜 남는 돈.\n" +
      "월세는 어차피 나가니까 빼고, 재료비만 뺀 금액.\n" +
      "이게 +면 팔수록 이득, −면 팔수록 손해.",
    intermediate:
      "판매단가에서 '변동비'만 뺀 값. 고정비(공장 감가상각·관리직 인건비 등)는 빠져 있습니다.\n" +
      "박리다매 가설의 핵심 엔진: 단위 총마진(고정비 포함)은 음수여도, 단위 공헌이익이 양수라면 '한 개 더 팔 때' 회사 전체 영업이익은 증가합니다 (고정비는 어차피 나가는 돈이라 추가 생산 시 증가하지 않는다는 가정).\n" +
      "단, 이 논리는 ①공장 캐파 여유 ②변동비 선형 증가 ③판가 실현 가능을 전제로 합니다.",
    expert:
      "출처: 100 보고서(판매단가·매출수량) + 200/501 보고서(변동비 14개 항목 합산). CVP(원가-조업도-이익) 분석의 핵심 지표. 실무 주의: (1) 단위 공헌이익 > 0이라도 단위 총마진 < 0이면 장기 고정비 회수 불가 → 구조적 적자. (2) 캐파 초과 시 'Step-up 고정비'(신규 라인 투자) 발생 → 모델 붕괴. (3) 경쟁사 반응·거래처 이탈·카니발라이제이션 미반영 시 박리다매 가설 위험.",
    benchmark:
      "< 0: 팔수록 현금 손실 (절대 금지)\n" +
      "0 ~ 변동비의 10%: 한계적, 충분한 물량 필요\n" +
      "변동비의 10% 이상: 건강한 공헌이익 (박리다매 여지)\n" +
      "⚠️ 단위 총마진(고정비 포함)과 반드시 함께 검토할 것",
    commonMistakes: [
      "'단위 마진'과 혼동 — 단위 마진은 총원가(고정비 포함) 기준, 단위 공헌이익은 변동비만 기준.",
      "단위 공헌이익 양수만 보고 '이 품목은 수익성 좋다'고 판단 — 장기적으로 고정비 회수 안 되면 사업 지속 불가.",
      "캐파 여유 확인 없이 '많이 팔수록 좋다'로 결론 — 캐파 초과 시 신규 설비 투자로 고정비 급증.",
      "경쟁사 반응 무시 — 저가 수주 성공 시 경쟁사도 가격 인하로 대응해 시장 공헌이익 전반 하락 가능.",
    ],
    relatedIds: ["volume_slider_4a", "price_slider_4a", "cost_slider_4a", "material_share"],
    source: ["100", "200", "501"],
    sourceNote:
      "100의 판매단가 · 200/501의 변동비 14개 항목(원재료·부재료·상품매입·노무·복리후생·소모품·수도광열·수선·연료·외주가공·운반·전력·지급수수료·견본비)을 합산하여 산출",
  },
  ...
  cannibalization: {
    ...
    unit: "ratio",
    formula:
      "ρ_AB = Pearson(품목 A 시계열, 품목 B 시계열)\n" +
      "ε_AB = β × meanB / meanA  (무차원 탄력성)\n" +
      "c_AB = max(0, -ρ × |ε|) × (1.5 if 같은 대분류 else 1.0)  → clamp [0, 1]",
    beginner:
      "🍰 우리 가게 빵을 더 팔면 우리 가게 케이크가 안 팔리는 효과.\n" +
      "한 회사 안에서 품목끼리 서로의 매출을 갉아먹는 현상.",
    intermediate:
      "같은 거래처가 우리 회사 품목 A를 더 사면 품목 B를 덜 사는 패턴(음의 상관)을 14개월 시계열에서 추출.\n" +
      "Pearson 상관계수로 강도를 측정하고, 같은 대분류 제품끼리는 1.5배 가중하여 잠식 가능성을 더 크게 봅니다.\n" +
      "단독 결정의 +효과가 다른 품목 매출을 잠식하면 포트폴리오 전체로는 +효과가 줄어들 수 있음.",
    expert:
      "Elasticity-weighted correlation: c = -ρ × |ε|, where ε = β × meanB/meanA. " +
      "샘플 ≥ 4M 필터(신뢰도 확보), 거래처별 ρ 산출 후 평균. " +
      "같은 거래처 내 품목 쌍만 분석(거래처 간 잠식은 WS6 경쟁사 반응에서 별도 처리). " +
      "한계: (1) 14M 데이터 → 신뢰도 low/medium/high 배지로 표시, (2) Pearson은 선형 관계만 포착(비선형은 v3), (3) 상관 ≠ 인과.",
    benchmark: "c < 0.1: 무시 가능 | 0.1-0.3: 경미한 잠식 | 0.3-0.6: 중간 | > 0.6: 강한 자기잠식",
    commonMistakes: [
      "양의 상관(ρ > 0)을 잠식으로 오해 — 양의 상관은 보완재(같이 잘 팔림) 신호, 잠식 0.",
      "거래처 간 잠식과 혼동 — 같은 회사 내 품목끼리만 분석. 경쟁사 반응은 WS6 분리 처리.",
      "샘플 < 4M 쌍은 매트릭스에서 제외됨 — 데이터 적은 품목 쌍은 신뢰도 낮음.",
    ],
    relatedIds: ["portfolio_net_effect", "cannibal_rate"],
    source: ["100"],
    sourceNote: "customerItemDetail (100, 거래처×품목×월 14개월 시계열)에서 Pearson 상관 매트릭스 추출",
  },
  ...
  cannibal_rate: {
    ...
    unit: "ratio",
    formula:
      "c_AB = min(1, max(0, -ρ × |β × meanB/meanA|) × categoryBoost)\n" +
      "categoryBoost = 1.5 if sameCategory(A, B) else 1.0\n" +
      "해석: target 매출 +X% → other 매출 −c × X%",
    beginner:
      "🔄 한 품목이 다른 품목을 갉아먹는 정도(0~1).\n" +
      "0이면 무관, 1이면 완전 대체재.",
    intermediate:
      "두 품목 간 자기잠식의 강도를 0~1로 정규화한 계수.\n" +
      "음의 상관(ρ<0)일 때만 0보다 크고, 같은 대분류 제품은 1.5배 가중 후 1로 클램핑.\n" +
      "예: c=0.3이면 'target 매출이 10% 증가하면 해당 품목 매출은 3% 감소'로 해석.",
    expert:
      "수식 분해: 상관강도(|ρ|) × 탄력성 크기(|ε|) × 카테고리 보정. " +
      "거래처별로 산출 후 산술평균(robust to single outlier). " +
      "샘플 < 4M 쌍 제외, 분산 0인 시계열(전월 동일)은 ρ=0 처리. " +
      "Pearson은 선형 관계만 포착하므로 비선형 잠식(예: 임계 가격 이하만 잠식)은 미반영.",
    benchmark: "0: 무관 / 보완재 | 0.1: 약한 잠식 | 0.3: 중간 잠식 | 0.6+: 강한 잠식 | 1.0: 완전 대체재",
    commonMistakes: [
      "c는 일방향이 아닌 페어(쌍) 단위 — (A,B)와 (B,A)는 별도 셀로 산출됨.",
      "c × 100 = 잠식률(%)이 아님 — c는 비율 변화의 비율(elasticity-style).",
      "1로 클램핑된 값은 '극단적 잠식 의심' 표시이지 '완전 대체재 확정'이 아님.",
    ],
    relatedIds: ["cannibalization", "portfolio_net_effect"],
    source: ["100"],
    sourceNote: "Pearson 상관계수(ρ) + 회귀 기울기(β) + 평균(meanA, meanB)로 산출",
  },

  portfolio_net_effect: {
    id: "portfolio_net_effect",
    name: "포트폴리오 순효과",
    category: "profitability",
    unit: "currency",
    formula:
      ...
      "단독효과 = 4a 시뮬의 netOffsetEffect (target 품목 결정 직접 효과)",
    beginner:
      "🌐 회사 전체 관점에서의 진짜 효과.\n" +
      "한 품목만 봤을 때 +였어도, 다른 품목을 갉아먹는 양을 빼야 진짜 +인지 알 수 있음.",
    intermediate:
      "단독 결정 효과에서 자기잠식 손실(다른 품목 매출 감소)을 차감한 회사 전체 순영향.\n" +
      "품목 A 가격 인하로 +6,319만 단독 효과지만, A가 B를 잠식해 -1,319만 손실 → 순효과 +5,000만.\n" +
      "실제 의사결정에서 가장 신뢰할 만한 지표 (단일 품목 관점의 함정 회피).",
    expert:
      "후처리 패턴: calcTotalViewSimulation 결과 + cannibalCorrection 입력 → portfolioNet 출력. " +
      "잠식 손실 환산: 매출 잠식의 70%를 영업이익 잠식으로 추정(변동비 30% 가정, 보수적). " +
      "프리셋 multiplier로 잠식 강도 조정(0.5/1.0/1.5/사용자). " +
      "WS7 12M 시뮬과 결합 시 12개월 누적 포트폴리오 NPV도 산출 가능.",
    benchmark:
      "단독 vs 순효과 차이 < 5%: 잠식 미미 | 5-20%: 일반 | 20%+: 포트폴리오 우려 큼",
    commonMistakes: [
      "단독 효과만 보고 의사결정 — 같은 카테고리 품목이 많으면 자기잠식이 크게 발생.",
      "0% 멀티플라이어로 카니발 보정 우회 — 보수적 시나리오는 약함(0.5)/중간(1.0)/강함(1.5) 비교 권장.",
      "잠식이 항상 손실은 아님 — 신제품으로 구제품 대체는 의도된 잠식 (포지셔닝 전략).",
    ],
    relatedIds: ["cannibalization", "cannibal_rate"],
    source: ["100"],
    sourceNote: "100 데이터 기반 단독 효과 + 카니발 매트릭스 보정",
  },
