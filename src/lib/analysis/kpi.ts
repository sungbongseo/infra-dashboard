...
export function calcCostStructure(teamContribData: TeamContributionRecord[]): CostStructureRow[] {
  // 월별 시트에서 같은 담당자가 여러 행으로 파싱되므로, (사번, 조직) 키로 합산
  const agg = new Map<string, {
    id: string; org: string; 매출액: number;
    원재료비: number; 상품매입: number; 외주가공비: number; 운반비: number;
    지급수수료: number; 노무비: number; 기타변동비: number; 고정비: number;
  }>();

  for (const r of teamContribData) {
    if (r.매출액.실적 === 0) continue;
    const key = `${r.영업담당사번}||${r.영업조직팀}`;
    const e = agg.get(key) || {
      id: r.영업담당사번, org: r.영업조직팀, 매출액: 0,
      원재료비: 0, 상품매입: 0, 외주가공비: 0, 운반비: 0,
      지급수수료: 0, 노무비: 0, 기타변동비: 0, 고정비: 0,
    };
    e.매출액 += r.매출액.실적;
    // 의도적 설계: Math.abs 미적용. 반제 전표로 음수 비용 발생 가능하며,
    // 차트에서 "수익 증가"처럼 보일 수 있으나 SAP 회계 원칙상 정상 처리
    e.원재료비 += r.제조변동_원재료비.실적 + r.제조변동_부재료비.실적;
    e.상품매입 += r.변동_상품매입.실적;
    e.외주가공비 += r.판관변동_외주가공비.실적 + r.제조변동_외주가공비.실적;
    e.운반비 += r.판관변동_운반비.실적 + r.판관변동_직접판매운반비.실적 + r.제조변동_운반비.실적;
    e.지급수수료 += r.판관변동_지급수수료.실적 + r.제조변동_지급수수료.실적;
    e.노무비 += r.판관변동_노무비.실적 + r.판관고정_노무비.실적 + r.제조변동_노무비.실적;
    // 판관고정_노무비는 노무비에 포함되므로 고정비에서 제외 (이중집계 방지)
    e.고정비 += r.판관고정_감가상각비.실적 + r.판관고정_기타경비.실적;
    e.기타변동비 += r.판관변동_복리후생비.실적 + r.판관변동_소모품비.실적 +
      r.판관변동_수도광열비.실적 + r.판관변동_수선비.실적 + r.판관변동_견본비.실적 +
      r.제조변동_복리후생비.실적 + r.제조변동_소모품비.실적 + r.제조변동_수도광열비.실적 +
      r.제조변동_수선비.실적 + r.제조변동_연료비.실적 + r.제조변동_전력비.실적 + r.제조변동_견본비.실적;
    agg.set(key, e);
  }

  return Array.from(agg.values()).map((e) => {
    const sales = Math.abs(e.매출액);
    const 원재료비율 = sales > 0 ? (e.원재료비 / sales) * 100 : 0;
    const 상품매입비율 = sales > 0 ? (e.상품매입 / sales) * 100 : 0;
    const 외주비율 = sales > 0 ? (e.외주가공비 / sales) * 100 : 0;
    const totalCost = e.원재료비 + e.상품매입 + e.외주가공비 + e.운반비 + e.지급수수료 + e.노무비 + e.기타변동비 + e.고정비;
    return {
      ...e,
      원재료비율,
      상품매입비율,
      외주비율,
      매출원가율: sales > 0 ? (totalCost / sales) * 100 : 0,
      ...(e.매출액 < 0 ? { hasNegativeSales: true } : {}),
    };
  });
}
