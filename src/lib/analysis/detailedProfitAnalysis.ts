...
export function calcParetoAnalysis(
  ...
  // 부동소수점 보정: 마지막 양수 항목의 cumShare 오차가 0.01% 미만일 때만 100%로 보정
  if (result.length > 0 && Math.abs(result[result.length - 1].cumShare - 100) < 0.01) {
    result[result.length - 1].cumShare = 100;
  }
...
export function calcProductGroupAnalysis(
  ...
  return Array.from(map.entries())
    .map(([group, v]) => ({
      ...
      exportRatio: Math.max(0, Math.min(100, safeDivide(v.exportSales, v.sales) * 100)),
